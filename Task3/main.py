...
class JobApplication(BaseModel):
    job_title: str
    company: str
    date_applied: str  # ISO format e.g. "2025-08-14"
    status: str
...
def add_application(application: JobApplication, current_user=Depends(get_current_user)):
...
def get_applications(current_user=Depends(get_current_user)):
    applications = load_json(APPLICATIONS_FILE)
    user_apps = [app for app in applications if app["username"]
